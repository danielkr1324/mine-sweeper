...
// Calculate cell size dynamically based on the number of cells
function setCellSize() {
  const boardContainer = document.querySelector('.board-container')
  const cellSize = boardContainer.clientWidth / gSize
...
//recursive function that expose content of one cell or more
...
function isValidCell(row, col) {
  // Check if the row and col are within the valid bounds of the board
  return row >= 0 && row < gBoard.length && col >= 0 && col < gBoard[0].length
}
...
function showCell(i, j) {
  ...
  gGame.showCount = gBoard[i][j].isShown ? gGame.showCount : gGame.showCount + 1
  gBoard[i][j].isShown = true
